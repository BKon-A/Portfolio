    ...
    public partial class ReportViewCustomers : Window
    {
        ...
        private void LoadReport()
        {
            try
            {
                ...
                var command = new SqlCommand("SELECT * FROM Customers_With_Paid_Repairs", connection);
                SqlDataReader dataReader = command.ExecuteReader();

                dataTable.Load(dataReader);
                dataReader.Close();

                reportViewer.LocalReport.ReportEmbeddedResource = "DBO_AWP_Computer_Service_Director.Report1.rdlc";
                reportDataSource = new("DataSet1", dataTable);
